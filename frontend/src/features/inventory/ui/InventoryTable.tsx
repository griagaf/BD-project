    ...
    <Card className="overflow-hidden p-0">
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={`${row.unitId}:${row.typeId}`} className="border-b border-zinc-900">
                <td className="px-4 py-3 text-zinc-100">{row.unitName}</td>
                <td className="px-4 py-3 text-zinc-400">{row.categoryName}</td>
                <td className="px-4 py-3 text-zinc-300">{row.typeName}</td>
                <td className="px-4 py-3 font-semibold text-zinc-100">{row.quantity}</td>
            ...
            {!rows.length ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-zinc-500">No inventory rows visible</td>
              </tr>
            ) : null}
          </tbody>
        </table>
